namespace SQLiteConsole_Local
{
    ...
    public class ChineseSpell
    {
        ...
        public static string convert(string chrstr)
        {
            ...
            cutstring = " ";
            isCh = false;
            return convert(chrstr, cutstring, out isCh);
        }

        public static string convert(string chrstr, string cutstring, out bool isCh)
        {
            ...
            for (int j = 0; j < nowchar.Length; j++)
            {
                ...
                if (array.Length == 1)
                { //英文字符
                    returnstr = string.Format("{0}{1}", returnstr, nowchar[j].ToString().ToLower());
                }
                else
                {
                    isCh = true;
                    ...
                    {
                        returnstr = string.Format(string.Format("{0}{1}", returnstr, nowchar[j]));
                    }
                    else
                    {
                        for (int i = (pyvalue.Length - 1); i >= 0; i--)
                        {
                            ...
                            {
                                if (m == 0)
                                    returnstr = string.Format("{0}{1}", returnstr, pystr[i]);
                                else
                                    returnstr = string.Format("{0}{2}{1}", returnstr, pystr[i], cutstring);
                                m++;
                                break;
                            }
                        }
